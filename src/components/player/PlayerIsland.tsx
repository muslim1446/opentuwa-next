...
export function PlayerIsland() {
  ...
  const handleProgressDown = useCallback((e: React.MouseEvent) => {
    ...
    const onMove = (ev: MouseEvent) => { if (draggingRef.current) handleProgressInteraction(ev.clientX) }
  ...
  const handleProgressTouch = useCallback((e: React.TouchEvent) => {
    ...
    const onMove = (ev: TouchEvent) => { if (draggingRef.current) handleProgressInteraction(ev.touches[0].clientX) }
    ...
    <div id="player-island">
      <div className="island-content">
        ...
        <div className="island-center">
          ...
          <span className="island-time-elapsed">{formatTime(currentTime)}</span>
        </div>

        <span className="island-time-remaining">-{formatTime(remaining)}</span>
      ...
      >
        <div className="island-progress-track">
          ...
          <div className="island-progress-thumb" style={{ left: `${progress}%` }} />
        </div>
      </div>
    </div>
  )
