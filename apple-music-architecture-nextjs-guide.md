...
export const metadata: Metadata = {
  ...
  other: {
    'apple-itunes-app': 'app-id=YOUR_APP_ID, app-argument=https://opentuwa.com/current/path',
  },
  icons: {
    icon: '/favicon.ico',
...
This is the one section I'm most confident is byte-for-byte accurate to spec, because it's the **official Open Graph protocol** (ogp.me), not something Apple invented — `og:type` literally has dedicated `music.song`, `music.album`, `music.playlist`, and `music.radio_station` types with their own property sets:
...
openGraph: {
  type: 'music.song',
  title: `${song.name} — Song by ${song.artistName} on ${SITE_NAME}`,
  url: songUrl,
...
```tsx
twitter: {
  card: 'summary_large_image', // standard share card
  title,
  description,
...
export function siteSearchJsonLd(): JsonLd {
  return {
    ...
    potentialAction: {
      '@type': 'SearchAction',
      target: 'https://opentuwa.com/search?term={search_term_string}',
...
export default async function sitemap({ id }: { id: number }): Promise<MetadataRoute.Sitemap> {
  const artists = await getArtistsPage({ offset: id * PER_SITEMAP, limit: PER_SITEMAP });
...
## 12. Applying This to a Quran Audio Platform

Mapping for a reciter/surah catalog (Tuwa-shaped), since that's the likely target:

| Apple Music concept | Quran audio equivalent | Best schema.org type |
|---|---|---|
| Artist | Reciter (Qari) | `MusicGroup` works for SEO purposes, but `Person` is more semantically correct for an individual reciter — consider emitting both `@type: ["Person", "MusicGroup"]` is invalid JSON-LD syntax, so instead pick `Person` and accept you lose some `MusicGroup`-specific rich-result eligibility, or stay on `MusicGroup` for the rich-result benefit and accept the slight semantic mismatch. This is a real tradeoff, not a settled convention — I'd lean `MusicGroup` purely for indexing benefit. |
| Album | Riwayah / full-Quran collection by a reciter | `MusicAlbum` |
| Song | Surah (individual chapter recitation) | `MusicRecording` is workable, but schema.org's `Chapter` type (used for audiobooks) is arguably a closer semantic fit — `Chapter` doesn't carry the same rich-result tooling as `MusicRecording` though, so this is another judgment call. If discoverability in audio rich results matters more than semantic purity, stay on `MusicRecording`. |
| Playlist | Curated reading plan / Juz' grouping | `MusicPlaylist`, or `ItemList` if you don't want the "music" framing |
| `isrc` | — (no equivalent) | Just omit; there's no recording-industry code for recitations |
| `previews[].url` | A short audio clip per surah | Same pattern — useful for `og:audio` / Twitter player cards |

The URL scheme translates directly: `/{reciter-slug}/{reciter-id}` → `/{collection-slug}/{collection-id}` → `/{surah-slug}/{surah-id}`, with `?i={surahId}` as the in-collection deep-link, exactly mirroring §1. Breadcrumbs become `Home → Reciter → Riwayah → Surah`.

One thing genuinely worth deciding before you commit to JSON-LD types: **does Google currently grant rich results to `MusicRecording`/`MusicAlbum` for religious-text recitations**, or could that read as schema misuse since the content isn't music in the colloquial sense? That's a judgment call about your content, not something I can settle from a stable spec the way the rest of this doc is — worth a quick Search Console test on a handful of pages with each typing before rolling it out catalog-wide.
